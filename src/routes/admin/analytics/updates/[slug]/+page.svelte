...
<div class="mb-6 flex flex-wrap items-center justify-between gap-3">
	...
	<div class="flex flex-wrap items-center gap-2">
		...
		{/if}
		<a href="/admin/updates/{data.post.slug}" class="btn text-sm">Edit update</a>
	</div>
</div>
...
<div class="mb-6 grid gap-4 lg:grid-cols-2">
	<div class="panel p-4 sm:p-5">
		...
		<AnalyticsDonutChart slices={sourceSlices} emptyLabel="No views for this update yet." />
	</div>
	<div class="panel p-4 sm:p-5">
		<h3 class="text-text mb-4 text-sm font-semibold tracking-wide uppercase">Source comparison</h3>
		<AnalyticsBarChart items={sourceBars} emptyLabel="No views for this update yet." />
	</div>
</div>
...
<section class="panel overflow-hidden">
	...
	{#if data.analytics.sourcesList.length === 0}
		<p class="text-text-muted p-4 text-sm">No views recorded for this update yet.</p>
	{:else}
